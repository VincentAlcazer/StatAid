  ...
  tagList(
   ...
   column(2,
          absolutePanel(width = 200, right = 20, draggable = T,
                        style = "opacity: 0.85",
                        wellPanel(
                          selectInput(ns("y_var"), label =("Y variable (outcome/to predict) - Numeric only"),
                                      multiple = F,selected = NULL,
                                      ""),
                          selectInput(ns("x_var"), label =("X Variable (explanatory/predictive) - Numeric only"),
                                      multiple = F, selected = NULL,
                                      choices=c("")),
                          radioButtons(ns("model"), label = ("Model"), 
                                             choices = list("Linear" = "lm", "GAM" = "gam", "LOESS" ="loess"),
                                             selected = "lm"),
                          radioButtons(ns("cor_type"), label = ("Correlation"), 
...
mod_Model_num_graph_coreg_server <- function(input, output, session, r){
  ...
  regression_df <- reactive({
    ...
    isolate({
      regression_dataframes(y_var=input$y_var, x_var=input$x_var, data = data(), model = input$model,
  ...
  smooth <- reactive({
    if(input$model == "gam"){
      ...
      geom_smooth(method="lm", se = T, color = "navyblue",
                            fill="lightblue", alpha = 0.5)
    } else if(input$model == "loess"){
      geom_smooth(method="loess", se = T, color = "navyblue",
                  fill="lightblue", alpha = 0.5)
    }
...
##### ===== OUTPUT   
    ...
    output$graph <- renderPlot({
      ...
      isolate({
        ...
        if(input$model == "lm"){
          title <- paste0("Linear model (Beta Coeff:", round(reg_param$estimate,3),
                          "; P-value: ", format.pval(reg_param$p.value,3,eps=0.0001),")")
        }else if (input$model == "gam"){
          title <- paste0("Generalized additive model (Edf:", round(reg_param$edf,3),
          "; P-value: ", format.pval(reg_param$p.value,3,eps=0.0001),")")
        }else if (input$model == "loess"){
    ...
    output$QQplot <- renderPlot({
      ...
      qplot(sample = Std_residuals, data = regression_df()[["augment_df"]]) +
        ...
        annotate(geom="text", x=-1.75, y=y_pos, label=paste0("Shapiro p: ",shapiro),
                 size = 5) +
        default_theme
