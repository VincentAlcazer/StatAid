...
mod_Explo_distri_ui <- function(id) {
  ...
  tagList(
    column(
      ...
      box(
        title = ("Summary table: count, NA & Shapiro test"),
...
mod_Explo_distri_server <- function(input, output, session, r) {
  ...
  output$Barchart <- renderPlot({
    data_explo() %>%
      ggbarplot(
        x = "Group", y = "variable", fill = "Group", size = 0.75, width = 1,
  ...
  output$Summary_na_table <- DT::renderDT(
    ...
    data_explo() %>%
      ...
      summarise(
        ...
        "Shapiro-Wilk pval" = ifelse(Count > 3, format.pval(shapiro.test(variable)$p.value, digits = 3, eps = 0.001), "NA")
      ), # data
