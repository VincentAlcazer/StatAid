  ...
  tagList(
    ...
    column(2,
           absolutePanel(width = 200, right = 20, draggable = T,
                         style = "opacity: 0.85",
                         wellPanel(
                           selectInput(ns("time_var"), label =("Time column (numeric)"),
                                       multiple = F,selected = NULL,
                                       ""),
                           selectInput(ns("y_var"), label =("Status column (0-1 or dead-alive)"),
                                       multiple = F,selected = NULL,
                                       ""),
                           selectInput(ns("x_var"), label =("X Variable (explanatory/predictive)"),
                                       multiple = F, selected = NULL,
                                       choices=c("")),
                           sliderInput(ns("x_cut"), label = ("Number of groups to cut X in (X numeric only)"), 
           ...
           box(title = "Graphical parameters", width = 11, collapsed = FALSE, collapsible = TRUE,
               column(4,
                      wellPanel(strong("Global"),
                                ...
                                sliderInput(ns("title_font_size"), label = "Title font size", min = 1, 
                                            max = 35, value = 20,step=1),
         
                                selectInput(ns("Colors"), label =("Colors panels"),
                                            choices=c("Default",
                                                      "Grey levels"= "grey",
                                                      "Nature" =  "npg",
                                                      "NEJM"=  "nejm",
                                ...
                                sliderInput(ns("risk_table_font_size"), label = "Font size (values - title)", min = 1, 
               ...
               column(4,
                      wellPanel(strong("X axis"),
                                ...
                                sliderInput(ns("x_font_size"), label = "Font size (values - title)", min = 1, 
               ...
               column(4,
                      wellPanel(strong("Y Axis"),
                                ...
                                sliderInput(ns("y_font_size"), label = "Font size (values - title)", min = 1, 
                                            max = 35, value = c(14,18),step=1)
                      )#Wellpanel
               ),#Column
               
               column(4,
                      wellPanel(strong("Legend"),
                                selectInput(ns("Legend"), label =("Position"),
                                            choices=c("Hide"= "none",
                                                      "Top"= "top",
                                                      "Right"=  "right",
                                                      "Left"=  "left",
                                ...
                                sliderInput(ns("legend_font_size"), label = "Font size (values - title)", min = 1, 
                                            max = 35, value = c(12,14),step=1)
                      )#Wellpanel
               ),#Column
               column(4,
                      wellPanel(strong("Statistics"),
                                ...
                                sliderInput(ns("pval_font_size"), label = "Font size", min = 0, 
                                            max = 35, value = 6),
                                sliderInput(ns("pval_x_coord"), label = "X position", min = 0, 
                                            max = 100, value = 1),
                                sliderInput(ns("pval_y_coord"), label = "Y position", min = 0, 
...
mod_Model_surv_km_server <- function(input, output, session, r){
  ...
  observe({
    updateTextInput(
      session,
      "legend_title", value=input$x_var)
  ...
  observe({
    ...
    updateSliderInput(
      session,
      "pval_x_coord", min = 0, max = max(surv_object()[["time"]]), step = max(surv_object()[["time"]])/100
    )
  ...
  output$km_curves <- renderPlot(
    ...
    ggsurvplot(surv_object(), fun = "pct",
               title=input$Title,
               ylab=input$y_lab,
               xlab=input$x_lab,
               #legend= c(0.8,0.3),
               ...
               ggtheme = theme_classic() +
                 theme(axis.title.x = element_text(size = input$x_font_size[2], face = "bold",color = "black"),
