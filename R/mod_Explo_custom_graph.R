    ...
    column(2, offset = 1,
           absolutePanel(width = 200, right = 20, draggable = T,
                         ...
                         wellPanel(strong("GRAPH TYPE"),
                                   ...
                                   selectInput(ns("Group_level"), label =("Group filter"),
...
mod_Explo_custom_graph_server <- function(input, output, session, r){
  ...
  observe({
    ...
    if(is.numeric(data()[,var_id]) == TRUE){
      ...
      updateSelectInput(
        session,
        "Graphtype",
        choices=c("Barchart (count)" = "Barchart_count",
                  "Histogram" = "Histogram") )
    }
  ...
  observe({
    updateTextInput(
      session, "y_lab",value=input$Variable
    )
    updateTextInput(
      session, "x_lab",value=input$Group
    )
  ...
  res <- reactive({
    ...
    isolate({
      ...
      res <- autoplot(data(), group=input$Group, 
